...
#         topk=10000,
        topk=arguments["filtered_log_probs"].shape[1], # size of vocab
    )
    msg += "\n" + return_msg
...
#         # --------for testing naturalness of fp
#         all_samples_with_mask = []
#         for fact in facts:
#             (sub, obj) = fact
#             sample = {}
#             sample["sub_label"] = sub
...
#                 args.template.strip(), sample["sub_label"].strip(), base.MASK
#             )
#             sample["masked_sentences"] = [args.template.strip().replace("[X]", base.MASK).replace("[Y]", base.MASK)]
#             all_samples_with_mask.append(sample)
#         # --------
...
#     # create uuid if not present
#     i = 0
#     for sample in all_samples_with_mask:
#         if "uuid" not in sample:
#             sample["uuid"] = i
#         i += 1    
    
    if args.fp:
#         MAX_FP_SEQ_LENGTH = 100
        
        import random
        from copy import deepcopy
        random.seed(0)
        all_samples_fp = deepcopy(all_samples)
#         all_samples_fp = deepcopy(all_samples_with_mask)
        num_samples = len(all_samples)
        samples_exluded_fp = 0
        
        for i,sample in enumerate(all_samples_fp):
            sampled_idx = random.sample(list(range(i))+list(range(i+1,num_samples)), k=10)
            
            masked_sentence = ", ".join([all_samples[j]['masked_sentences'][0].replace(base.MASK, all_samples[j]['obj_label'])[:-1] for j in sampled_idx])
            
            masked_sentence_fp = masked_sentence+", "+sample['masked_sentences'][0]
            
            sample['masked_sentences'] = [masked_sentence_fp]
            
        
        all_samples = all_samples_fp
